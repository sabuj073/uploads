...
listenChange(".product_all", function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.success) {
                ...
                var $variationsSelect = element.parent().parent().find("td .variations")
                $variationsSelect.empty();
                $.each(result.data.variations, function (key, value) {
                    $variationsSelect.append('<option value="' + value + '">' + value + '</option>');
                });
